...
def extract_m3u8(url):
    """
    Extract m3u8 links from a URL using Playwright with retry on timeout.
    ...
    """
    def try_navigate(url, timeout_ms, attempt):
        try:
            with sync_playwright() as p:
                ...
                page.wait_for_timeout(3000)  # Reduced from 5s
    ...
    result = try_navigate(url, 60000, 1)
    if result is not None and result:
        return result

    print("🔄 Retrying with extended timeout...")
    result = try_navigate(url, 90000, 2)
